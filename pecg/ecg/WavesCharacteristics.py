...
def compute_amp(ecg, features_dict):
    ...
    if len(features_dict) == 1:
        amp[~np.isnan(features_dict[0])] = ecg[features_dict[0][~np.isnan(features_dict[0])]]

    else:
        if len(features_dict[0][~np.isnan(features_dict[0])]) == 0 or \
                len(features_dict[1][~np.isnan(features_dict[1])]) == 0:
            return np.nan
        begin_amp = np.ones(len(features_dict[0])) * np.nan
        end_amp = np.ones(len(features_dict[0])) * np.nan
        begin_amp[~np.isnan(features_dict[0])] = ecg[features_dict[0][~(np.isnan(features_dict[0]))].astype(int)]
        end_amp[~np.isnan(features_dict[1])] = ecg[features_dict[1][~(np.isnan(features_dict[1]))].astype(int)]
