...
def fecgyn_tgen(ecg, qrs, fs):
    ...
    phase_change_points = np.asarray(
        np.where((phase[1:] < 0) & (phase[:-1] > 0))
    ).squeeze()
    n_cycles = len(phase_change_points)
    cycle_index = np.arange(
        phase_change_points[start_cycle] + 1, phase_change_points[start_cycle + 1] + 1
    )
    cycle_f = scipy.interpolate.splrep(phase[cycle_index], ecg[cycle_index])
    cycle = np.expand_dims(
        scipy.interpolate.splev(np.linspace(-PI, PI, n_bins), cycle_f), 1
    )
    Mode = {
        "cycles": {1: cycle},
        "mean_cycle": {1: cycle},
    ...
    while (nc_relev_mode < MIN_nc) & (Th > MIN_TRESH):
        ...
        for i in range(start_cycle + 1, n_cycles - 2):
            ...
            cycle_index = np.arange(
                phase_change_points[i] + 1, phase_change_points[i + 1] + 1
            )
            cycle_f = scipy.interpolate.splrep(phase[cycle_index], ecg[cycle_index])
            cycle = np.expand_dims(
                scipy.interpolate.splev(np.linspace(-PI, PI, n_bins), cycle_f), 1
            ...
            while (not match) & (ind_mode <= n_modes):
                coeff = np.corrcoef(
                    cycle.squeeze(), Mode["mean_cycle"][ind_mode].squeeze()
                )
            ...
            if not match:
                ...
                Mode["cycles"][ind_mode - 1] = np.concatenate(
                    (Mode["cycles"][ind_mode - 1], cycle), axis=1
                )
                Mode["mean_cycle"][ind_mode - 1] = np.mean(
                    Mode["cycles"][ind_mode - 1], axis=1
                )
                Mode["std_cycle"][ind_mode - 1] = np.std(
                    Mode["cycles"][ind_mode - 1], axis=1
                )
                Mode["nc"][ind_mode - 1] = Mode["nc"][ind_mode - 1] + 1
                Mode["cycle_len"][ind_mode - 1] = np.hstack(
                    (Mode["cycle_len"][ind_mode - 1], len(cycle_index))
                )

        rel_mode_ind = []
        for i in Mode["nc"]:
            if (Mode["nc"][i] > n_rel) & (
                np.mean(Mode["cycle_len"][i]) >= MIN_TLEN * fs
            ):
        ...
        if not len(rel_mode_ind):
            ...
            mean_cycle_len = sum(Mode["cycle_len"][max_ind]) / len(
                Mode["cycle_len"][max_ind]
            )
            template["avg"] = scipy.signal.resample(
                template["avg"], int(np.round(mean_cycle_len))
            )
            template["std"] = scipy.signal.resample(
                template["std"], int(np.round(mean_cycle_len))
            )
        Th = Th - PACE
    return template
